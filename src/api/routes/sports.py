"""Sports tracking routes: Programs and conversations.

This module handles sports training programs with dedicated conversations
where the AI acts as a personal trainer. Program definitions, goals, and
progress are stored in the K/V store (namespace: sports).
"""
...
@api.route("/sports/programs", methods=["GET"])
@api.output(SportsProgramsResponse)
@api.doc(responses=[401, 429])
@rate_limit_conversations
@require_auth
def list_programs(user: User) -> dict[str, Any]:
    """List user's sports training programs."""
    programs = _get_programs(user.id)

    # Check which programs have conversations
    sports_convs = db.list_sports_conversations(user.id)
    conv_programs = {c.sports_program for c in sports_convs}
...
@api.route("/sports/programs", methods=["POST"])
@api.output(SportsProgramsResponse)
@api.doc(responses=[400, 401, 429])
@rate_limit_conversations
@require_auth
@validate_request(CreateSportsProgramRequest)
def create_program(user: User, data: CreateSportsProgramRequest) -> dict[str, Any]:
    """Create a new sports training program."""
...
def delete_program(user: User, program_id: str) -> dict[str, Any]:
    ...
    db.delete_sports_conversation(user.id, program_id)

    # Clean up KV data for this program
    for suffix in ("goals", "preferences", "routine", "progress", "last_session"):
...
@api.route("/sports/<program>/conversation", methods=["GET"])
@api.output(SportsConversationResponse)
@api.doc(responses=[401, 404, 429])
@rate_limit_conversations
@require_auth
def get_sports_conversation(user: User, program: str) -> dict[str, Any]:
    """Get or create a sports conversation for a program."""
    ...
    conv = db.get_or_create_sports_conversation(user.id, program, model=Config.DEFAULT_MODEL)
...
@api.route("/sports/<program>/reset", methods=["POST"])
@api.output(SportsResetResponse)
@api.doc(responses=[401, 404, 429])
@rate_limit_conversations
@require_auth
def reset_sports_conversation(user: User, program: str) -> dict[str, Any]:
    """Reset a sports conversation (delete messages + clear checkpoint)."""
    # Verify program exists
    programs = _get_programs(user.id)
    if not any(p["id"] == program for p in programs):
        raise_not_found_error("Program")

    result = db.reset_sports_conversation(user.id, program)
    if not result:
        raise_not_found_error("Sports conversation")

    return {
        "success": True,
        "message": "Sports conversation reset successfully",
    }
