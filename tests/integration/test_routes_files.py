...
def _message_with_image(
    ...
    conv = test_database.create_conversation(user.id, "Files test", "gemini-3.7-flash")
...
class TestGetMessageThumbnail:
    ...
    def test_non_image_file_is_rejected(
        ...
        conv = test_database.create_conversation(test_user.id, "Docs", "gemini-3.7-flash")
        message = test_database.add_message(
            conv.id,
            MessageRole.USER,
            "a document",
...
def _message_with_video(test_database: Database, user: User, days_old: int = 0) -> str:
    ...
    conv = test_database.create_conversation(user.id, "Video test", "gemini-3.7-flash")
    message = test_database.add_message(
        conv.id,
        MessageRole.USER,
        "here is a video",
...
class TestExpiredMediaGone:
    ...
    def test_fresh_pdf_still_served(
        ...
        conv = test_database.create_conversation(test_user.id, "Docs2", "gemini-3.7-flash")
        message = test_database.add_message(
            conv.id,
            MessageRole.USER,
            "a document",
