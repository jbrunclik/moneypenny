...
describe('initDeepLinking and cleanupDeepLinking', () => {
  ...
  it('returns null conversationId when no conversation in hash', () => {
    ...
    expect(result).toEqual({ conversationId: null, isPlanner: false, isAgents: false, isStorage: false, isSports: false });
  });
  ...
  it('returns conversation ID when present in hash', () => {
    ...
    const callback = vi.fn();
    const result = initDeepLinking(callback);
    expect(result).toEqual({ conversationId: 'conv-123', isPlanner: false, isAgents: false, isStorage: false, isSports: false });
  });
  ...
  it('returns planner route info when on planner hash', () => {
    ...
    const callback = vi.fn();
    const result = initDeepLinking(callback);
    expect(result).toEqual({ conversationId: null, isPlanner: true, isAgents: false, isStorage: false, isSports: false });
  });
...
describe('hashchange event handling', () => {
  ...
  it('calls callback with conversation ID on hashchange', async () => {
    ...
    expect(callback).toHaveBeenCalledWith('conv-456', false, false, false, false);
  });
  ...
  it('calls callback with null for non-conversation hash', async () => {
    ...
    expect(callback).toHaveBeenCalledWith(null, false, false, false, false);
  });
  ...
  it('calls callback with isPlanner=true for planner hash', async () => {
    ...
    expect(callback).toHaveBeenCalledWith(null, true, false, false, false);
  });
  ...
  it('ignores hashchange during programmatic updates', async () => {
    ...
    expect(callback).not.toHaveBeenCalledWith('conv-789', false, false, false, false);
  });
});
