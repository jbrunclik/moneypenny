...
- [ ] **Webkit search-spec flake under full-suite load** - 2 occurrences (Jun 2026), still unexplained after a deep hunt (Jun 12):
  ...
  - Mitigation (Aug 28 2026): the webkit project gets **3** CI retries vs chromium's 2 (`playwright.config.ts`, per-project `retries`). This is a false-red suppressant for these load-correlated webkit flakes, not a root-cause fix - both flake entries stay open. Retries only run on failure, so the green path is untaxed; workers stay pinned at 4 (see the tuned-throughput comment in the config).
  - **Revisit (target ~mid-Sep 2026, or after ~20-30 main-branch CI runs):** did the retry bump actually stop the false reds? Check `gh run list --branch main --workflow Tests` for E2E jobs that went red then passed on rerun. If flakes still redden CI at 3 retries -> the failure isn't retry-recoverable under sustained load; escalate to dropping E2E workers 4->3 (reduces peak contention) and/or pull a fresh trace to finally root-cause. If zero false reds and CI feels reliably green -> consider this settled and close both webkit-flake entries.
