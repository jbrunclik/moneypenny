...
- [ ] **Streaming requests killed by gunicorn worker timeout** - Long agent runs (many tool calls / image gen / browser+code tools) on the SSE `chat/stream` path can exceed gunicorn's `--timeout` (systemd default 600s). With **sync** workers the timeout is wall-clock for the whole request and SSE keepalives don't reset it (the worker only heartbeats the master between requests), so the master kills the worker mid-stream. The kill arrives as `SystemExit` (a `BaseException`), which bypasses `except Exception` in `chat_streaming.py:659` but still runs the `finally`, deleting the placeholder → generated content lost (see X1). **Fix: switch to `--worker-class gthread --threads N`** (keeps heartbeat alive during long requests); raising `GUNICORN_TIMEOUT` alone only moves the cliff. Files: `systemd/ai-chatbot.service`. Note: gthread shares memory across threads per worker — confirm per-request state stays thread-safe (kv_store, not module dicts).
