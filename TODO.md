...
- [ ] **Eval tool-round counts drifted UP ~19% since Sep 6 2026** (measured Sep 15 2026 on a clean network, single sample per case). Total rounds across the suite went **32 -> 38**; 11 of 34 comparable cases moved, in both directions: `research_multi_source` 2 -> 6 (now sitting exactly on its cap of 6), `cz_local_lookup` 3 -> 5 and `cz_episodic_recall` 3 -> 5 (both now FAIL their cap of 4), `web_lookup_cited` 2 -> 5, `sports_zwo_generate` 1 -> 3 — against `delegate_deep_research` 5 -> 2 and `sports_progression_plan` 3 -> 1 going the other way. Quality is unaffected (every one of those scored 5/5); this is efficiency only. **Ruled out**: the Sep 15 search changes (commit 4e16fc0) — all their behavioural effects are gated behind `is_degraded()` and the degraded path fired **zero** times across three eval runs. **Candidates**: live-model drift on `gemini-3.8-flash`, or the five dependency bumps that reached main between Sep 6 and Sep 15 without main CI. **Caveat**: one sample per case and the variance is clearly large (the same case flipped pass/fail across runs on the same day), so do not act on this alone — fold it into the tool-round verification item above (due ~Sep 20), which measures real traffic rather than 34 single samples. If the drift is real, the affected caps (`max_tool_rounds` 4 on the two failing cases) are also worth re-examining: several cases now sit exactly one round over.
