...
- [ ] **Verify cost drop after a week of data** - rerun `scripts/analyze_costs.py` on the prod host (~Jun 19); cached share of input should be substantial, daily cost trending down, and cost-weighted `tool_rounds`/turn falling as the model adopts batched/parallel search. If cache_read stays 0, debug the cached_content path.
- [ ] **Reduce search round-multiplication behaviorally** - the model still does several single-search rounds (partially batches via native parallel tool-calls). The `queries[]` param overlaps with parallel calling; the real lever is prompting it to commit all searches up front. Consider a soft per-turn tool-round cap with an "answer with what you have" nudge once `tool_rounds` telemetry confirms which conversations are worst.
