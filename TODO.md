...
- [ ] **Agent-behavior observability (A3, remainder)** - Mostly shipped Aug 2026: eval harness at 33/33 (`make eval`, usage-grounded cases incl. sports mode, memory reads, attachments, episodic recall - see [docs/testing/evals.md](docs/testing/evals.md)) and per-turn metrics on `message_costs` (duration_ms, tool_errors, tools_used; `analyze_costs.py` Turn Metrics section). Remaining: a delegate_task eval case, and revisit turn-metric trends after a few weeks of data.
...
- [ ] **Vendor chunk splitting** - Aug 2026 lazy-loading pass cut boot from 220 kB gz (single 773 kB chunk) to ~196 kB gz + on-demand view chunks (Settings/AgentEditor/CommandCenter/KVStore). The remaining 604 kB shared chunk mixes rarely-changing vendors (markdown-it, highlight.js, katex, zustand) with app code - a `manualChunks` vendor split would make deploys cache-friendlier (vendors unchanged across most releases). Also: agents.ts's dynamic import of conversation.ts is a cycle-breaker that still logs INEFFECTIVE_DYNAMIC_IMPORT noise at build.
