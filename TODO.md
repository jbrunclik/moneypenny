...
- [ ] **`research_multi_source` eval regressed 2 -> 7 tool rounds (Sep 15 2026)** - the MacBook comparison case passed at **2 rounds** using only `research` in the Sep 6 baseline; on Sep 15 it used `research` AND `web_search` across **7 rounds**, busting its `max_tool_rounds: 6` cap. Consistent across both runs that day, and quality stayed 5/5 - this is efficiency, not correctness. **Ruled out**: the Sep 15 search changes (commit 4e16fc0). All of their behavioural effects are gated behind `is_degraded()`, and the degraded path never fired during the eval run (zero "degraded to DuckDuckGo" log lines); the atomic breaker fix only trips exhausted providers *sooner*, which removes HTTP calls rather than adding LLM rounds. **Still open**: whether this is live-model drift on `gemini-3.8-flash` (the tool-round work is explicitly a behavioural bet on a live model - see the verification item above), fallout from the five dependency bumps that landed between Sep 6 and Sep 15 without main CI, or a real prompt-surface problem. Cheapest next step: `git worktree` at `94808a7` and run `evals/run.py --only research_multi_source` on both commits a couple of times (~$0.06/run) to separate model drift from repo change.
